...
**TConstrain** is an R package for semi-supervised single-cell trajectory inference. It's a preprocessor-like tool to overcome the limitations of purely unsupervised methods by allowing researchers to incorporate biological prior knowledge—such as time points, known differentiation pathways, or experimental perturbations—as topological constraints. (**This package is not fully developed.**)

## **Why TConstrain?**

Current state-of-the-art pseudotime methods (e.g., Slingshot, Monocle 3, PAGA) rely on transcriptomic similarity to infer developmental time. While powerful, this assumption lack flexibility in several biological scenarios:
...
TConstrain modifies the Minimum Spanning Tree (MST) construction step used in standard algorithms. Instead of relying solely on Euclidean distance, it accepts a user-defined Constraint Table:
